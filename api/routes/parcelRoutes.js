...
router.route('/:id/done').patch(restrictTo(EMP),addVarBody("status","delivered"), parcelController.updateParcel);
router.route('/:id/location').patch(restrictTo(USER), parcelController.location);
router
.route('/mine')
.get(restrictTo(USER),addQuery('userId', 'userId') , parcelController.getAllParcel);
